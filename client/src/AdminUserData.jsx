...
export default function AdminUserData() {
    ...
    const handleLogout = async () => {
        try {
            const { res } = await apiFetchCurrent("auth/logout", {
                method: "POST",
            });
            if (res.ok) {
                setLoggedIn(false);
    ...
    const handleLogoutAll = async () => {
        try {
            const { res } = await apiFetchCurrent("auth/all-logout", {
                method: "POST",
            });
            if (res.ok) {
                setLoggedIn(false);
    ...
    const uploadFileToServer = async (pId, filename) => {
        ...
        try {
            const response = await fetch(
                appendUserId(`${BASE_URL}/upload/${filename}`),
                {
                    method: "POST",
                    headers: {
    ...
    const createFolderOnServer = async () => {
        ...
        try {
            const response = await fetch(
                appendUserId(`${BASE_URL}/directory/${foldername}`),
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                    ...
                    <tbody className="divide-y divide-zinc-800">
                        {dirItems.directories.map((dir) => (
                            <DirectoryRow
                                ...
                                readOnly={!allowCrud}
                                linkBuilder={(id) =>
                                    `/admin/users/${userId}/data/directory/${id}`
                                }
                            />
                        ))}
