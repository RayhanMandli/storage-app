...
import { logFileOperation, logError, logSecurity, logInfo } from "../utils/logger.js";
  ...
  try {
    ...
    writeStream.on('error', (error) => {
      ...
      File.deleteOne({ _id: newFile._id }).catch(err => {
        logError('Failed to cleanup file record after write error', err);
      });
    ...
    writeStream.on("finish", async () => {
      ...
      logFileOperation('upload', req.user._id, newFile._id, filename, true);
...
export const serveFileController = async (req, res, next) => {
  ...
  try {
    ...
    // Log file access
    logFileOperation(action === "download" ? "download" : "view", 
                    req.user._id, id, file.filename, true);
